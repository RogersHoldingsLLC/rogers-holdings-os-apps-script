...
function getFollowUpRuleForStatus_(status) {
  const rules = {
    'Lead Found': { type: 'Website Audit', days: 0, priority: 'A - Hot', notes: 'Run website audit today.' },
    'Audit Complete': { type: 'Proposal', days: 1, priority: 'A - Hot', notes: 'Prepare proposal tomorrow.' },
    'Proposal Sent': { type: 'Follow-Up', days: 3, priority: 'A - Hot', notes: 'Follow up after proposal sent.' },
    ...
    'Discovery Scheduled': { type: 'Discovery Reminder', days: 1, priority: 'A - Hot', notes: 'Reminder one day before discovery call.', useDiscoveryDate: true },
    'Won': { type: 'Client Welcome Task', days: 0, priority: 'A - Hot', notes: 'Welcome client and confirm onboarding next steps.' },
    'Lost': { archive: true }
  };
...
function formatFollowUpsSheet_(sheet, headers) {
  ...
  sheet.setFrozenRows(findBestHeaderRow_(sheet));
  sheet.setTabColor(ROGERS_OS_THEME.gold);
  sheet.getRange(findBestHeaderRow_(sheet), 1, 1, Math.max(sheet.getLastColumn(), FOLLOW_UP_COLUMNS.length))
    .setBackground(ROGERS_OS_THEME.black)
    .setFontColor(ROGERS_OS_THEME.gold)
    .setFontWeight('bold')
  ...
  applyWrapByHeader_(sheet, headers, findBestHeaderRow_(sheet), sheet.getLastRow(), ['Notes']);
  applyColumnFormattingByHeader_(sheet, headers, findBestHeaderRow_(sheet), sheet.getLastRow(), {
...
function updateDashboardKPIs_(data) {
  ...
  const rows = [
    ...
    [data.averageAuditScore, '', '', data.clientMetrics.totalClients || 0, '', '', data.clientMetrics.activeClients || 0, '', '', data.clientMetrics.activeProjects || 0, '', '']
  ];
