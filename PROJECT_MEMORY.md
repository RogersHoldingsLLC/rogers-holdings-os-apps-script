...
The DE-002 identity exclusion snapshot v1 is implemented locally as a separate BOP-owned, strictly read-only request version. It opens only the workbook identified by `BOP_SPREADSHEET_ID` and requires its exact title to match the explicit `HEADQUARTERS_IDENTITY_EXPORT_EXPECTED_WORKBOOK_TITLE` property, with no active-workbook or title fallback. This preserves exact production binding while allowing the approved non-production title to be configured for disposable acceptance through the same code path. It requires two identical complete read-only acquisitions of the documented row-4 Master Prospect Tracker and Clients sources, builds the exact response, and then requires a matching third complete acquisition as its final source operation. It rejects configuration or source mutation through that final boundary, sparse/null/undefined entry input, header/data formulas, malformed or globally duplicated IDs, contract-invalid business names/domains, more than 100,000 entries, and canonical JSON beyond 512 KiB. Its exact Headquarters-compatible response exposes only version/source/completeness/freshness plus record ID, literal lifecycle, business name, and a zero-or-one canonical domain array. It uses the separate `HEADQUARTERS_IDENTITY_EXPORT_TOKEN` property and does not change Headquarters Sales Feed v1 semantics. Local focused and regression validation is required in this milestone; no Script Property, token, live access, Apps Script call, acceptance deployment, production deployment, or Headquarters enablement is authorized. See `HEADQUARTERS_IDENTITY_EXPORT_V1.md`.
