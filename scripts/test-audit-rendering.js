...
const auditPdfPaginationFunction = pdfSource.slice(pdfSource.indexOf('function buildAuditReportPdfBlob_'), pdfSource.indexOf('function buildProposalPdfBlob_'));
...
assert.match(pdfSource, /Improvement Roadmap', priorityRoadmapHtml_\(prospect, safeReportFile\), \{[\s\S]*?keepWithFirstBlock: true,[\s\S]*?forcePageBreak: true,[\s\S]*?pageTopSafe: true[\s\S]*?\}/, 'Improvement Roadmap begins on an intentional page and remains attached to its first content block');
assert.match(pdfSource, /'Timeline', projectRoadmapHtml_\(\), \{ keepWithFirstBlock: true, forcePageBreak: true, pageTopSafe: true \}/, 'Improvement Plan Timeline remains attached to its first item and clears the repeating page header');
...
assert.match(pdfSource, /opts\.forcePageBreak[\s\S]*sectionClasses\.push\('force-page-break'\)/, 'page breaks are explicit and independent from heading keep rules');
...
assert.match(pdfSource, /renderPdfSectionIntroGroup_\(\s*'IMPROVEMENT ROADMAP',\s*'Business Impact'/, 'Improvement Roadmap divider and introduced content render as one page-safe section');
assert.doesNotMatch(auditPdfPaginationFunction, /renderPdfCompactDivider_\('IMPROVEMENT ROADMAP'\)/, 'Improvement Roadmap is not emitted as an isolated divider');
assert.match(pdfSource, /function renderPdfSectionIntroGroup_[\s\S]*?<div class="section section-intro-group">/, 'Roadmap introduction flows into available space instead of creating a substantially empty standalone page');
assert.doesNotMatch(pdfSource, /function renderPdfSectionIntroGroup_[\s\S]*?<div class="section force-page-break section-intro-group">/, 'Roadmap introduction does not force a substantially empty content page');
assert.match(pdfSource, /\.conclusion-panel\.audit-conclusion \{ break-inside: auto; page-break-inside: auto; \}/, 'assessment next-step content may fill the qualified service-package page instead of leaving it substantially empty');
assert.match(pdfSource.slice(pdfSource.indexOf('function finalRecommendationHtml_'), pdfSource.indexOf('function buildRecommendedPackage_')), /conclusion-panel audit-conclusion/, 'assessment final recommendation opts into intentional cross-page flow');
assert.match(pdfSource, /\.audit-discovery-close \{ break-before: page; page-break-before: always; \}/, 'assessment discovery close begins as an intentional final-page block');
assert.match(pdfSource.slice(pdfSource.indexOf('function finalRecommendationHtml_'), pdfSource.indexOf('function buildRecommendedPackage_')), /audit-discovery-close[\s\S]*final-metrics[\s\S]*Schedule a short discovery conversation[\s\S]*contact-card/, 'assessment final page keeps metrics, discovery action, and contact details together in sequence');
