...
function normalizeExecutiveBusinessEvidence_(context) {
  ...
  executiveBusinessArray_(report.evidence).forEach(function(item) { add(item, { source: 'Website Audit Tool' }); });
  executiveBusinessArray_(report.findings).forEach(function(item) {
    add(item, { source: 'Website Audit Tool', supportsOpportunity: true });
  });
