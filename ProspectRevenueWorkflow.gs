...
function ensureProspectRevenueId_(context) {
  ...
  const generated = 'PROS-' + Utilities.getUuid().replace(/-/g, '').slice(0, 12).toUpperCase();
...
function runProspectRevenuePreparation_(isResume) {
  ...
  try {
    ...
    if (!isVerifiedAuditDataForLocalRendering_(prospect)) {
      ...
      logProspectRevenueActivity_(context, 'Website Inspection Complete', 'Verified Website Audit Tool evidence was recorded.', operationKey);
    } else {
      logProspectRevenueActivity_(context, 'Website Inspection Reused', 'Existing verified Website Audit Tool evidence was reused.', operationKey);
    }
...
function generateExecutiveSnapshotForRevenueContext_(context, prospect) {
  ...
  upsertAuditPackageBlobFile_(folder, 'Executive Snapshot.pdf', buildExecutiveSnapshotPdfBlob_(prospect, reportFile));
  logPipelineActivity_(context.ss, prospect.company, 'Executive Snapshot Generated', 'Generated through Prospect-to-Revenue Workflow v1.');
}
...
function recordProspectRevenueReviewDecision_(decision) {
  ...
  setIfHeaderCell_(context.sheet, context.table.headers, context.selectedRow, 'Next Action', decision === 'Approved' ? 'Confirm Executive Snapshot Sent' : 'Create Outreach Draft');
  updateSelectedProspectLastActivity_(context.sheet, context.table.headers, context.selectedRow);
  logProspectRevenueActivity_(context, 'Brian Review ' + decision, notes || 'Review decision recorded. No email was sent.', operationKey);
  refreshSalesOperatingSystem_();
  ui.alert(decision, decision === 'Approved' ? 'Send the Gmail draft manually, then run Confirm Executive Snapshot Sent.' : 'Update the record, then run Resume Prospect Workflow.', ui.ButtonSet.OK);
...
function logProspectRevenueActivity_(context, activityType, activityNotes, operationKey) {
  ...
  schema.activitySheet.getRange(row, 1, 1, schema.activityTable.lastColumn).setValues([rowValues]);
}
